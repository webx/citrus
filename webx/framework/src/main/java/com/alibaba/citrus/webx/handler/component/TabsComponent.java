...
public class TabsComponent extends PageComponent {
    ...
    private class TabsVisitor extends AbstractVisitor {
        ...
        public void visitTab(Template tabTemplate) {
            for (TabItem tab : tabs) {
                tabTemplate.accept(new TabVisitor(context, tab, tabTemplate));
            }
        }
    }
    ...
    private class TabVisitor extends AbstractVisitor {
        ...
        private final Template tabTemplate;

        public TabVisitor(RequestHandlerContext context, TabItem tab, Template tabTemplate) {
            super(context, TabsComponent.this);
            this.tab = assertNotNull(tab, "tab");
            this.tabTemplate = assertNotNull(tabTemplate, "tabTemplate");
        }

        public void visitSelected(Template selectedTemplate) {
            if (tab.isSelected()) {
                selectedTemplate.accept(this);
            }
        }

        public void visitHref(Template hrefTemplate) {
            if (tab.getHref() != null) {
        ...
        public void visitOnclick(Template onclickTemplate) {
